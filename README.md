...
Heatmaps were generated for the three region using `pheatmap` (R package). The sample-distance heatmap shows pairwise Euclidean distances between samples based on VST expression values. The DE gene heatmap shows VST expression of the significant differentially expressed genes.  Genes are hierarchically clustered while samples remain in original order. 
